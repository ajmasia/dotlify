...
cmd_doctor::run() {
  ...
  if ((BASH_VERSINFO[0] < 4)); then
    ...
    ui::warn "$(printf "${MSG_DOCTOR_BASH_OLD:-bash < 4.0 detected (found: %s)}" "${BASH_VERSION}")"
    issues=$((issues + 1))
  fi
  ...
  if ! command -v stow >/dev/null 2>&1; then
    ...
    if [[ -n "$stow_ver" ]]; then
      ...
      if ((major < 2)) \
        ...
        # shellcheck disable=SC2059
        ui::warn "$(printf "${MSG_DOCTOR_STOW_OLD:-stow < 2.3.1 detected (found: %s)}" "$stow_ver")"
        issues=$((issues + 1))
      fi
    fi
  fi

  # Broken symlinks under $HOME pointing into $DOTS_DIR.
  ...
  while IFS= read -r -d '' link; do
    ...
    if [[ -n "$link_target" && "$link_target" == "${dots_dir}"* && ! -e "$link" ]]; then
      # shellcheck disable=SC2059
      ui::warn "$(printf "${MSG_DOCTOR_BROKEN_LINK:-Broken symlink: %s}" "$link")"
      issues=$((issues + 1))
    fi
  ...
  if ((issues == 0)); then
    ...
    # shellcheck disable=SC2059
    ui::warn "$(printf "${MSG_DOCTOR_ISSUES:-%s issue(s) found.}" "$issues")"
  fi
}
